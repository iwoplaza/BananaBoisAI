...
input_dir = str(os.getcwd())[:-4] + '/DATASET/validation_small'
answers_file = str(os.getcwd())[:-4] + '/RESULTS/file.csv'
...
def task_1(partial_output: dict, file_path: str) -> dict:
    ...
    pred = validation(file_path)

    for idx, label in enumerate(labels_task_1):
        partial_output[label] = int(pred[0, idx])
    # ==========================================
